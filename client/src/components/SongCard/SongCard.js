...
const SongCard = ({ song }) => {
    ...
    const handleMusicStream = () => {
        if (audio) {
            audio.paused ? 
            handlePlay() && setPaused(false) : handlePause() && setPaused(true);
        } else {
        ...
        <div className="bg-gray-900 shadow-lg rounded-lg p-3 w-52 hover:bg-gray-800 transition ">
            <div className="group relative">
                ...
                <div className="absolute p-2 bg-black rounded bg-opacity-0 group-hover:bg-opacity-20 w-full h-full top-0 flex items-center group-hover:opacity-100 transition justify-evenly">
                    ...
                    <button onClick={ audio && paused === false ? handlePause : handleMusicStream } className="hover:scale-110 text-white opacity-0 transform translate-y-3 group-hover:translate-y-0 group-hover:opacity-100 transition">
