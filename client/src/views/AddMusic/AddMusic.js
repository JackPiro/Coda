...
const AddMusic= ({fileList, setFileList, releaseType, setReleaseType}) => {
    ...
    return (
        <div className="bg-gradient-to-r from-[#0E121A] to-indigo-700">
            <SideNavBar />
            ...
            <div className="flex w-full min-w-screen min-h-screen">
                <div className="bg-[#181C25] mt-6 mr-6 mb-6 text-white rounded-3xl shadow-xl w-full overflow-hidden flex items-center ml-56 justify-center p-10 space-x-6">
                    ...
                    <div className="bg-[#202530] rounded-lg flex flex-col items-center justify-between w-1/2 h-full p-5">
                        ...
                        <button onClick={fileList.length > 0 ? () => navigate('/finish-adding-music') : null} className="p-2 mt-4 register-button">Continue</button>
                    </div>
                </div>
            </div>
        </div>
